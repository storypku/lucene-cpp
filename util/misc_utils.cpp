...
uint64_t MiscUtils::get_time_millis(boost::posix_time::ptime time) {
    return boost::posix_time::time_duration(time - boost::posix_time::ptime(boost::gregorian::date(1970,
                                            1, 1))).total_milliseconds();
}
...
int64_t MiscUtils::unsigned_shift(int64_t num, int64_t shift) {
    return (shift & 0x3f) == 0 ? num : (((uint64_t)num >> 1) & 0x7fffffffffffffffLL) >> ((
                shift & 0x3f) - 1);
}
